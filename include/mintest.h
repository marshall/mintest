...
#define _mt_assert_msg(message, test) do { if (!(test)) return mt_msg(message); } while (0)
#define mt_fail(message) return mt_msg(message)
#define mt_assert(test) mt_assert_msg(mt_str(test), test)

#if defined(MT_ENABLE_TRACE)

#    define mt_trace(message, ...) \
         mt_printf("mintest: TRACE: " message MT_NL, ## __VA_ARGS__)

#    define mt_assert_msg(message, test) do { \
         mt_trace(mt_msg(message)); \
        _mt_assert_msg(message, test); \
     } while(0)

#else // !defined(MT_ENABLE_TRACE)

#    define mt_trace(message, ...) ((void) 0)
#    define mt_assert_msg _mt_assert_msg

#endif // defined(MT_ENABLE_TRACE)
...
#    define mt_report_test_begin(test) mt_printf("mintest:     Running " \
                                                 mt_str(test) ".." MT_NL);
#    define mt_report_test_end(test)
#    define mt_report_suite_end(suite)
#    define mt_report_end()

#    define mt_report_fail(test, message) mt_printf("mintest: FAIL  : " #test \
                                                    ": %s" MT_NL, message)
#    define mt_report_counts() \
         mt_printf("mintest: %d passed, %d failed, %d total" MT_NL, \
                   mt_pass_count, mt_fail_count, mt_tests_run)
...
#    define mt_report_fail(test, message) \
         mt_printf("%s{\"test\":\"%s\",\"message\":\"%s\"}", \
                   mt_fail_count > 1 ? "," : "", mt_str(test), message)

#    define mt_report_counts() \
         mt_printf("], \"passed\": %d,\"failed\":%d,\"total\":%d", \
              mt_pass_count, mt_fail_count, mt_tests_run)
